...
let cents = function
  | Zero -> 0.
  | Equal_division_of_the_octave { divisor; parts } ->
    1200. *. float_of_int parts /. float_of_int divisor
...
let add t1 t2 =
  match t1, t2 with
  | Zero, t | t, Zero -> t
  | ( Equal_division_of_the_octave { divisor = d1; parts = p1 }
    , Equal_division_of_the_octave { divisor = d2; parts = p2 } )
    when d1 = d2 -> Equal_division_of_the_octave { divisor = d1; parts = p1 + p2 }
...
let rec of_symbolic (symbolic : Symbolic.t) =
  ...
  | Equal_tempered_12 interval ->
    ...
    Equal_division_of_the_octave { divisor = 12; parts = number_of_semitons }
